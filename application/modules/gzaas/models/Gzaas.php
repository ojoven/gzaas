...
class Gzaas_Model_Gzaas
{
    ...
    public function previewGzaas($parameters)
    {
        ...
        $launcher    = My_Functions::getVariableFromArrayOrNullIfIndexIsNotSet($parameters,'launcher');
    ...
    public function renderGzaas($urlKey)
    {
        ...
        $launcherModel = new Gzaas_Model_Launcher();
        $launcher = $launcherModel->getMessageLauncher($message['id']);
    ...
    private function _cleanLauncher($launcher)
    {
        if (trim($launcher) != '') {
            $launcher = stripSlashes($launcher);
            $launcher = htmlspecialchars($launcher, ENT_NOQUOTES);
            if (strlen($launcher) > self::LAUNCHER_MAX_SIZE) {
                $launcher = substr($launcher, 0, self::LAUNCHER_MAX_SIZE);
                return $launcher;
            }
            return $launcher;
        } else {
    ...
    private function _addLauncherIfLauncher($launcher, $idMessage)
    {
        if ($launcher != '') {
            $launcherModel = new Gzaas_Model_Launcher();
